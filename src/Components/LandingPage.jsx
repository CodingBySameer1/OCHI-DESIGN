...
const   LandingPage = () => {
  const locomotiveScroll = new LocomotiveScroll();
    ...
    return (
        <div  data-scroll data-scroll-section  data-scroll-speed="-0.7" className=' h-screen  w-full'>
          <div className='flex items-end  justify-between' >
          <div className='pt-[12vw] px-[4vw] w-fit font-["Founderxbold"]  ' >
                {["We Create", "Eye-Opening", "Presentations"].map((h1, index) => {
                    return (
                        <div className=' flex items-center ' >
                            {index === 1 && (<motion.div initial={{ width: 0 }} animate={{ width: "9vw" }} transition={{ease : [0.87, 0, 0.13, 1]}} className='w-[0vw] h-[6vw] relative bottom-[1.8vw] mr-[0.7vw] bg-red-500 rounded-[0.4vw]  overflow-hidden' >
                                <img src="https://ochi.design/wp-content/uploads/2022/04/content-image01.jpg" className='w-full h-full object-cover' alt="" />
                            </motion.div>)}
                            <h1 key={index} className='text-[9vw] leading-[6.9vw] uppercase from-neutral-50' >
                                {h1}
                            </h1>
                        </div>);
                })}
            </div>
            <div className='site of the day mb-[8.4vw]  '>
            ...
            <div className='border-t-1 flex flex-col md:flex-row items-center   justify-between px-[4vw] border-zinc-400 mt-[9.5vw] '>
                {["For public and private companies", "From the first pitch to IPO"].map((h3, index) => {
                    return (
                        <h3 key={index} className={`capitalize mt-[1.4vw] font-["Founderlight"] text-md  ${index === 1 && "ml-[11vw]" }  `} >{h3}</h3>
                    )
                })}
                <div className='flex  items-center gap-[1vw]'>
                    <button className='border-1 px-[1vw] rounded-full ' >START THE PROJECT</button>
                    <span className='border-1 p-[0.4vw]  rounded-full -rotate-45 font-light' >
                        <FaArrowRight />
                    </span>
                </div>
            </div>
